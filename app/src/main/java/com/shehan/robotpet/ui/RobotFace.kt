...
@Composable
fun RobotFace(emotion: Emotion, gazeX: Float, gazeY: Float, onTouch: () -> Unit, modifier: Modifier = Modifier) {
    ...
    Canvas(modifier.fillMaxSize().pointerInput(Unit) { detectTapGestures { onTouch() } }) {
        val w = size.width
        val h = size.height
        val eyeW = w * 0.23f
        ...
        val glow = Color(0xFF7DE7FF)

        fun drawEye(c: Offset, rotation: Float) {
            rotate(rotation, c) {
                drawRoundRect(
                    color = glow.copy(alpha = 0.12f),
                    topLeft = Offset(c.x - eyeW * 0.56f, c.y - eyeH * 0.62f),
                    size = Size(eyeW * 1.12f, eyeH * 1.24f),
                    cornerRadius = CornerRadius(eyeH * 0.48f)
                )
                drawRoundRect(
                    color = glow,
                    topLeft = Offset(c.x - eyeW / 2, c.y - eyeH / 2),
                    size = Size(eyeW, eyeH.coerceAtLeast(5f)),
